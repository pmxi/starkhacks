...
import { ChevronLeft, Settings, Timer, Dumbbell, Coins, Zap, Info } from 'lucide-react';
...
export default function GameSettings() {
  ...
  // Sync external settings changes (from host's broadcasts)
  useEffect(() => {
    if (room?.settings) {
  ...
  const handleConfirm = () => {
    const settings = { reps, timeLimit, entryFee };
    emitStartGame(settings);
    navigate('/gameplay');
  };
      ...
      <div className="w-full max-w-[390px] h-full flex flex-col p-6 relative">
        ...
        <div className="flex items-center gap-2 mb-6 px-1">
          ...
          <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">
            {isHost ? 'You are configuring — changes sync live' : 'Host is configuring settings...'}
          </span>
        </div>
        ...
        <div className="flex-1 space-y-8 overflow-y-auto no-scrollbar pb-6">
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              value={reps}
              disabled={!isHost}
              onChange={e => handleSettingsChange('reps', parseInt(e.target.value))}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[30, 60, 90, 120].map(t => (
                <button
                  key={t}
                  onClick={() => isHost && handleSettingsChange('timeLimit', t)}
                  disabled={!isHost}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => isHost && handleSettingsChange('entryFee', parseFloat(Math.max(0.05, entryFee - 0.05).toFixed(2)))}
                disabled={!isHost}
              ...
              <input
                ...
                value={entryFee}
                disabled={!isHost}
                onChange={e => handleSettingsChange('entryFee', parseFloat(e.target.value))}
                className="flex-1 h-2 bg-white/5 rounded-full appearance-none cursor-pointer accent-[#b794f6] disabled:opacity-50"
              />
              <button
                onClick={() => isHost && handleSettingsChange('entryFee', parseFloat(Math.min(2.0, entryFee + 0.05).toFixed(2)))}
                disabled={!isHost}
                className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 text-white font-black flex items-center justify-center hover:bg-white/10 transition-all disabled:opacity-50"
              >
                +
            ...
            <div className="relative z-10 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  ...
                  <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">5% network fee included</span>
                </div>
              </div>
              <div className="text-right">
                <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em] block mb-1">Squad</span>
                <div className="text-xl font-black text-white italic">{playerCount}/6</div>
                <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">{room.gameType}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pb-4">
          {isHost ? (
            ...
            <div className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-6 flex items-center justify-center gap-3">
              <div className="w-4 h-4 border-2 border-[#b794f6]/30 border-t-[#b794f6] rounded-full animate-spin" />
