...
export function GameProvider({ children }: { children: React.ReactNode }) {
  const [playerName, setPlayerNameState] = useState<string>(() =>
    localStorage.getItem('solfit_name') || ''
  );
  const [playerId] = useState<string>(() => {
    const stored = localStorage.getItem('solfit_id');
    if (stored) return stored;
    const id = generateId();
    localStorage.setItem('solfit_id', id);
    return id;
  });
  ...
  const [stats, setStats] = useState<Stats>(() => {
    try {
      const stored = localStorage.getItem('solfit_stats');
  ...
  const setPlayerName = useCallback((name: string) => {
    ...
    localStorage.setItem('solfit_name', name);
  }, []);
  ...
  const updateStats = useCallback((reps: number, solWon: number, gameType: string) => {
    setStats(prev => {
      ...
      localStorage.setItem('solfit_stats', JSON.stringify(newStats));
      return newStats;
    });
  }, []);
  ...
  const leaveRoom = useCallback(() => {
    ...
    if (r && socket) {
      socket.emit('leave-room', { code: r.code, playerId });
    }
    setRoom(null);
  }, [playerId, socket]);
  ...
  const emitRepUpdate = useCallback((count: number) => {
    const r = roomRef.current;
    if (r && socket) {
      socket.emit('rep-update', { code: r.code, playerId, count });
    }
  }, [playerId, socket]);
  ...
  const emitGameEnd = useCallback((results: Array<{ id: string; name: string; count: number; isYou: boolean }>) => {
    const r = roomRef.current;
    if (r && socket) {
      socket.emit('game-end', { code: r.code, results });
    }
  }, [socket]);
  ...
  return (
    <GameContext.Provider value={{
      playerName, setPlayerName, playerId,
