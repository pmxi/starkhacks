...
export default function CreateTeam() {
  ...
  return (
    <div className="min-h-full bg-[#0a0a0f] overflow-y-auto">
      <div className="w-full max-w-[390px] md:max-w-2xl mx-auto flex flex-col p-6 md:p-10 min-h-full">
        ...
        {/* Host info */}
        <div className="flex items-center gap-3 bg-white/5 border border-white/5 rounded-2xl px-4 py-3 mb-8">
              ...
              <div className="grid grid-cols-3 gap-3">
                {(['Pushup', 'Squat', 'Plank'] as GameType[]).map(type => (
                  <button
                    ...
                    disabled={!!inviteCode}
                    className={`py-4 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all disabled:cursor-default flex flex-col items-center gap-1 ${
                      gameType === type
                  ...
                  >
                    <span>{type}</span>
                    <span className="text-[8px] opacity-60 normal-case font-medium">{GAME_TYPE_DESC[type]}</span>
                  </button>
                ))}
              </div>
            </div>
              ...
              {!inviteCode ? (
                <>
                  <button
                    ...
                    className="w-full bg-white/5 border border-dashed border-white/20 rounded-2xl py-10 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    {isGenerating ? (
                      <div className="w-5 h-5 border-2 border-[#b794f6]/30 border-t-[#b794f6] rounded-full animate-spin" />
                    ) : (
                      <Zap className="w-6 h-6 text-[#b794f6] group-hover:scale-110 transition-transform" />
                    )}
                    <span className="text-[10px] font-black text-[#b794f6] uppercase tracking-[0.2em]">
                      {isGenerating ? 'Creating Room...' : 'Generate Invite Code'}
                    </span>
                    {!teamName.trim() && (
                      <span className="text-[9px] text-white/20 font-medium">Enter team name first</span>
                    )}
                  </button>
                  {error && (
